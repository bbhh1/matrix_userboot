...
async def _(event):
    if ispay[0] == "yes":
        await event.edit("** ◇︰حسنا صديقي اشترك بقنوات الاجباري لكي لايواجهك خطا .**")
        ...
        for i in range(100):
            ...
            try:
                ...
                chs += 1
                await jepiq.send_message(event.chat_id, f"◇︰تم الاشتراك في : {chs} قنوات .")
            except:
                await jepiq.send_message(event.chat_id, f"خطأ , ممكن تبندت")
                break
        await jepiq.send_message(event.chat_id, "**◇︰ تم الانتهاء من تجميع النقاط .**")
