...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**⌯︙المستخدم** [{jepthon}](tg://user?id={user.id}) \n**⌯︙ تـم رفعـه بقلبي 🤍 بواسطة :** {my_mention} \n**⌯︙ انت حبي الابدي 😍**")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**⌯︙المستخدم** [{jepthon}](tg://user?id={user.id}) \n**⌯︙ تـم رفعـه اهبل 🐴 بواسطة :** {my_mention} \n**⌯︙ تعال ياهبل استلم العربيه **")
    
#كـتابة المـلف وتعديل.    :   امير .   اخمط وسمي روحك مطور فرخي 😂
# اذا انت ابن حرام اخمط 😂
# اي بعدك تريد تخمط ترا من تخمط مراح تنجح
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**⌯︙المستخدم** [{jepthon}](tg://user?id={user.id}) \n**⌯︙ تـم رفع المتهم زاحف اصلي بواسطة :** {my_mention} \n**⌯︙ ها يلزاحف متى تبطل سوالفك حيوان 😂🐍**")

@jepiq.on(admin_cmd(pattern="رفع قحبه(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**⌯︙المستخدم** [{jepthon}](tg://user?id={user.id}) \n**⌯︙ تـم رفع المتهم قحبه 👙 بواسطة :** {my_mention} \n**⌯︙ ها يالقحبه تعال خلي انيكك/ج**")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**⌯︙المستخدم** [{jepthon}](tg://user?id={user.id}) \n**⌯︙ تـم رفعـها حاته الكروب 🤤😻 بواسطة :** {my_mention} \n**⌯︙ تعال يعافيتي اريد حضن دافي 😽**")

@jepiq.on(admin_cmd(pattern="رفع يمني(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**⌯︙المستخدم** [{jepthon}](tg://user?id={user.id}) \n**⌯︙ تـم رفعـه من جنوبي اليمن الى شمالي اليمن ♕ بواسطة :** {my_mention} \n**⌯︙ ها يا الجنوبي  خليك شمالي  😂**")
...
@jepiq.on(admin_cmd(pattern="رفع مقوت(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f" ** ⌯︙ المستخدم => • ** [{jepthon}](tg://user?id={user.id}) \n ☑️ **⌯︙ تم رفعه مقوتي بواسطه  :**{my_mention} .\n**⌯︙ قوم يالمقوتي قوت لاتسوي شيخ عندي هههههه 😹** ")
...
async def OwnerStart(event):
    ...
    if sender.id == ownersayed_id :
        order = await event.reply('لبيه مطوري امير ❤')
ownersayed1_id = 5214682327
@jepiq.on(events.NewMessage(outgoing=False, pattern='امير؟'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownersayed1_id :
        order = await event.reply('  يب منصب ✓ ❤️')
ownersayed1_id = 5214682327
@jepiq.on(events.NewMessage(outgoing=False, pattern='انس؟'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownersayed1_id :
 order = await event.reply('  ملك الضلام')
